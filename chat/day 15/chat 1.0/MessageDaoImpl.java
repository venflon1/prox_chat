...
//				java.sql.Date dateSql = result.getDate("message_date");
//				java.util.Date dateMess = new java.util.Date(dateSql.getTime());
//				LocalDateTime localDatetime = dateMess.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
				messageToRetrieve.setMexTime(Util.String2LocalDateTime(result.getString("message_date")));
	...
	public List<Message> findByDate(Long time) {
		...
		try {
			...
			while(resultSet.next()){
				...
				m.setUserName(resultSet.getString("username"));
				m.setTextMessage(resultSet.getString("text"));
				m.setMexTime(Util.String2LocalDateTime(resultSet.getString("message_date")));
